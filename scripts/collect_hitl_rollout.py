#!/usr/bin/env python3
"""Collect human-in-the-loop rollouts and save them as a robomimic-style HDF5 dataset.

Starting from a pretrained policy (the data-collection / rollout policy), this runs HITL rollouts
exactly like ``scripts/train_hitl.py`` collection phase — keyboard/SpaceMouse teleop, or a simulated
human (frozen expert + intervention criterion) — and writes the collected trajectories to an HDF5
file in the layout consumed by :class:`H5ReplayBuffer`:
...
Notes on conventions (to stay compatible with H5ReplayBuffer):
  * Observations are stored RAW: the collection worker z-scores low-dim obs for the policy's act(),
    so we un-normalize them back with the same stats before writing. Downstream you can therefore
    load with ``normalize_lowdim_obs=True`` (recomputes stats from the data) just like any dataset.
  ...
  * The per-step ``intervention`` dataset preserves the HITL label that MILE / DiffusionMILE need.
    The stock H5ReplayBuffer only supports a single ``default_intervention_label`` and IGNORES this
    per-demo dataset; a small loader extension (added when wiring train_hitl_offline.py) is required
    to surface it as ``info['intervention']``.

``load_dir`` points at a *pretraining run output directory* (same as train_hitl.py): env / training /
model / policy are read from ``load_dir/.hydra/config.yaml`` and the actor from
``load_dir/checkpoints/<step>/actor.pt``.

Usage:
    uv run python scripts/collect_hitl_rollout.py --config-name robomimic_collect_hitl \
        load_dir=/path/to/dp_run hitl.human_mode=simulated \
        hitl.expert_load_dir=/path/to/expert_run \
        hitl.collect_num_rollouts=50 hitl.collect_output_path=/path/to/hitl_rollouts.hdf5
"""
...
def main(cfg: DictConfig):
    ...
    # ---- Intervention source: keyboard/spacemouse teleop, or a simulated human. ----
    ...
    if human_mode == "simulated":
        ...
        if not expert_dir:
            raise ValueError("hitl.human_mode=simulated requires hitl.expert_load_dir.")
        ...
        for cfg_key, kw, cast in (
            ...
            ("hitl.criteria_stochastic", "stochastic", bool),
            ("hitl.criteria_threshold", "threshold", float),
        ):
    ...
    # ---- Pre-collection sanity eval (simulated human only): evaluate the rollout (initial) policy
    # and the simulated expert over `pre_collection_eval_episodes` episodes BEFORE collecting, so we
    # can confirm the expert is meaningfully better than the rollout policy. If they perform
    # similarly, the simulated corrections are near-in-distribution and give little/no HITL training
    # signal -- regenerate with a stronger expert or a weaker rollout policy. Uses a separate chunked
    # eval env (like train_hitl.py) so chunked actors execute open-loop. ----
    if human_mode == "simulated" and not debug:
        ...
        logger.info(f"Pre-collection eval (100 eps): rollout (initial) policy...")
        rollout_metrics = eval_worker.run(actor)
        logger.info(f"Pre-collection eval (100 eps): simulated expert...")
        expert_metrics = eval_worker.run(expert_policy)
        r_sr = float(rollout_metrics.get("success_rate", float("nan")))
        e_sr = float(expert_metrics.get("success_rate", float("nan")))
        logger.info(
            f"Pre-collection eval: rollout success_rate={r_sr:.3f}, expert success_rate={e_sr:.3f}, "
            f"gap={e_sr - r_sr:+.3f}"
        )
        if e_sr - r_sr < 0.1:
            logger.warning(
                "Simulated expert is NOT meaningfully better than the rollout policy (gap < 0.10). "
                "Its corrections will be near-in-distribution, giving little/no HITL training signal "
                "(flat MILE losses, non-discriminating probit). Use a stronger expert or a weaker "
                "rollout policy before collecting."
            )
    ...
    logger.info(
        ...
        f"(human_mode={human_mode}, store_only_human={store_only_human}, "
        f"keep_only_hitl_rollouts={keep_only_hitl}) -> {output_path}"
    )
    ...
    # ---- Segment analysis of the collected data: intervention rate + human-segment length
    # distribution vs chunk_size (short segments => diluted/dropped label-1 chunks => weak signal). ----
    analyze_intervention_segments(
        online_buffer,
        chunk_size=OmegaConf.select(cfg, "training.chunk_size", default=None),
        context="collected",
    ...
    meta_extra = {
        ...
        "intervention_criteria": str(OmegaConf.select(cfg, "hitl.intervention_criteria", default=None)),
        "store_only_human": store_only_human,
    ...
    logger.success(
        f"Wrote {stats['num_demos']} ({attempt} attempts) demos / {stats['total_transitions']} transitions "
